...
import {
  APNT_CREATION_SCOPE_V2_STATEMENT_VERSION,
  APNT_CREATION_SCOPE_V2_VERSION,
  APNT_TRANSITION_BATCH_NONCE_V2_VERSION,
  deriveAPNTCreationScopeV2,
  deriveAPNTTransitionBatchNonceV2,
} from "./apnt_creation_scope_v2.js";
import {
...
export const APNT_TRANSITION_STATEMENT_V2_MAGIC = "APNTTSV2";
export const APNT_TRANSITION_STATEMENT_V2_VERSION = 2;
export const APNT_TRANSITION_STATEMENT_V2_PROTOCOL_DOMAIN = "bch-cloak-apnt-v0";
export const APNT_TRANSITION_STATEMENT_V2_COMMITMENT_DOMAIN =
  "bch-cloak-apnt-v0:transition-statement-commitment-v2";
export const APNT_TRANSITION_STATEMENT_V2_MAX_LOGICAL_NOTES_PER_SIDE = 1_024;
export const APNT_TRANSITION_STATEMENT_V2_MAX_BACKING_CELLS_PER_SIDE = 4_096;
/** Canonical encoding of an absent designated aggregator-fee output index. */
export const APNT_TRANSITION_AGGREGATOR_FEE_OUTPUT_INDEX_ABSENT_V2 = 0xffff_ffff;

export const APNT_TRANSITION_MODE_V2 = Object.freeze({
  PRIVATE_TRANSITION: "private-transition",
  COMPLETE_BUNDLE_EXIT: "complete-bundle-exit",
} as const);
export type APNTTransitionModeV2 =
  (typeof APNT_TRANSITION_MODE_V2)[keyof typeof APNT_TRANSITION_MODE_V2];

export type APNTTransitionNetworkV2 = "chipnet" | "mainnet" | "regtest";

export type APNTTransitionConsumedLogicalNoteV2 = Readonly<{
  consumedNoteCommitment32: Bytes32;
  nullifier32: Bytes32;
}>;

export type APNTTransitionConsumedBackingCellV2 = Readonly<{
  inputIndex: number;
  sealCellCommitment32: Bytes32;
  lockingProfileId32: Bytes32;
}>;

export type APNTTransitionCreatedLogicalNoteV2 = Readonly<{
  createdNoteCommitment32: Bytes32;
  recoveryPacketIndex: number;
  recoveryPacketHash32: Bytes32;
}>;

export type APNTTransitionCreatedBackingCellV2 = Readonly<{
  outputIndex: number;
  sealCellCommitment32: Bytes32;
  lockingProfileId32: Bytes32;
  /**
 ...
 */
export type APNTTransitionStatementV2 = Readonly<{
  version: typeof APNT_TRANSITION_STATEMENT_V2_VERSION;
  protocolDomain: typeof APNT_TRANSITION_STATEMENT_V2_PROTOCOL_DOMAIN;
  network: APNTTransitionNetworkV2;
  mode: APNTTransitionModeV2;
  ...
  consumedLogicalNotes: readonly APNTTransitionConsumedLogicalNoteV2[];
  consumedBackingCells: readonly APNTTransitionConsumedBackingCellV2[];
  createdLogicalNotes: readonly APNTTransitionCreatedLogicalNoteV2[];
  createdBackingCells: readonly APNTTransitionCreatedBackingCellV2[];
  /** V1 fixed-slot recovery packet-bin root; required only for private-transition. */
  recoveryPacketBinRoot32: Bytes32 | null;
  /**
...
const MAGIC = Uint8Array.of(0x41, 0x50, 0x4e, 0x54, 0x54, 0x53, 0x56, 0x32); // APNTTSV2
...
function assertBchValue(name: string, value: unknown): bigint {
  if (typeof value !== "bigint" || value < 0n || value > APNT_V1_BCH_MAX_MONEY_SATS) {
    throw new Error(`${name} must be a non-negative BCH bigint within the v2 money range`);
  }
  return value;
}
...
function normalizeNetwork(name: string, value: unknown): APNTTransitionNetworkV2 {
...
function normalizeMode(name: string, value: unknown): APNTTransitionModeV2 {
  if (
    value !== APNT_TRANSITION_MODE_V2.PRIVATE_TRANSITION &&
    value !== APNT_TRANSITION_MODE_V2.COMPLETE_BUNDLE_EXIT
  ) {
...
function assertArrayWithinCap(
  ...
  if (value.length > cap) throw new Error(`${name} exceeds the v2 codec cap`);
}

function normalizeConsumedLogicalNote(
  name: string,
  value: unknown,
): APNTTransitionConsumedLogicalNoteV2 {
...
function normalizeConsumedBackingCell(
  name: string,
  value: unknown,
): APNTTransitionConsumedBackingCellV2 {
  assertRecord(name, value);
...
function normalizeCreatedLogicalNote(
  name: string,
  value: unknown,
): APNTTransitionCreatedLogicalNoteV2 {
  assertRecord(name, value);
...
function normalizeCreatedBackingCell(
  name: string,
  value: unknown,
): APNTTransitionCreatedBackingCellV2 {
  assertRecord(name, value);
  assertKnownKeys(name, value, [
    "outputIndex",
...
function normalizeConsumedLogicalNotes(value: unknown): readonly APNTTransitionConsumedLogicalNoteV2[] {
  const name = "APNTTransitionStatementV2.consumedLogicalNotes";
  assertArrayWithinCap(name, value, APNT_TRANSITION_STATEMENT_V2_MAX_LOGICAL_NOTES_PER_SIDE);
  const notes = value
...
function normalizeConsumedBackingCells(value: unknown): readonly APNTTransitionConsumedBackingCellV2[] {
  const name = "APNTTransitionStatementV2.consumedBackingCells";
  assertArrayWithinCap(name, value, APNT_TRANSITION_STATEMENT_V2_MAX_BACKING_CELLS_PER_SIDE);
  const cells = value
...
function normalizeCreatedLogicalNotes(value: unknown): readonly APNTTransitionCreatedLogicalNoteV2[] {
  const name = "APNTTransitionStatementV2.createdLogicalNotes";
  assertArrayWithinCap(name, value, APNT_TRANSITION_STATEMENT_V2_MAX_LOGICAL_NOTES_PER_SIDE);
  const notes = value
...
function normalizeCreatedBackingCells(value: unknown): readonly APNTTransitionCreatedBackingCellV2[] {
  const name = "APNTTransitionStatementV2.createdBackingCells";
  assertArrayWithinCap(name, value, APNT_TRANSITION_STATEMENT_V2_MAX_BACKING_CELLS_PER_SIDE);
  const cells = value
...
function assertProjectionJoins(
  ...
  if (
    ...
    throw new Error(
      "APNTTransitionStatementV2 consumed backing cells must exactly cover private-backing projection inputs",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "APNTTransitionStatementV2 created backing cells must exactly cover private-backing projection outputs",
    );
  }
}

/**
 * Closes the public fee-output shape without selecting a Profile V2 fee rule.
 * Profile-specific rate arithmetic remains a downstream relation concern.
 */
function assertAggregatorFeeShape(statement: APNTTransitionStatementV2): void {
  const feeOutputs = statement.transactionProjection.outputs.flatMap((output, index) =>
    output.role === APNT_TRANSITION_OUTPUT_ROLE_V1.AGGREGATOR_FEE
      ? [{ output, index }]
      : []
  );
  if (statement.aggregatorServiceFeeSats > 0n) {
    if (feeOutputs.length !== 1) {
      throw new Error(
        "APNTTransitionStatementV2 positive aggregatorServiceFeeSats requires exactly one aggregator-fee projection output",
      );
    }
    const designated = feeOutputs[0]!;
    if (statement.aggregatorFeeOutputIndex !== designated.index) {
      throw new Error(
        "APNTTransitionStatementV2.aggregatorFeeOutputIndex must select the sole aggregator-fee projection output",
      );
    }
    if (designated.output.valueSats !== statement.aggregatorServiceFeeSats) {
      throw new Error(
        "APNTTransitionStatementV2 aggregator-fee projection output value must equal aggregatorServiceFeeSats",
      );
    }
    return;
  }
  if (statement.aggregatorFeeOutputIndex !== null || feeOutputs.length !== 0) {
    throw new Error(
      "APNTTransitionStatementV2 zero aggregatorServiceFeeSats must not contain or designate an aggregator-fee output",
    );
  }
}

function assertModeShape(statement: APNTTransitionStatementV2): void {
  assertAggregatorFeeShape(statement);
  if (statement.mode === APNT_TRANSITION_MODE_V2.PRIVATE_TRANSITION) {
    if (
      ...
      throw new Error("APNTTransitionStatementV2 private-transition collections must be non-empty");
    }
    if (statement.createdBackingCreationScope32 === null) {
      throw new Error("APNTTransitionStatementV2 private-transition requires a creation scope");
    }
    if (statement.recoveryPacketBinRoot32 === null) {
      throw new Error("APNTTransitionStatementV2 private-transition requires a recovery packet bin root");
    }
    if (statement.transactionProjection.outputs.some(
      (output) => output.role === APNT_TRANSITION_OUTPUT_ROLE_V1.TRANSPARENT_EXIT,
    )) {
      throw new Error("APNTTransitionStatementV2 private-transition must not contain transparent-exit outputs");
    }
    if (!statement.transactionProjection.outputs.some(
      (output) => output.role === APNT_TRANSITION_OUTPUT_ROLE_V1.RECOVERY_PACKET_CARRIER,
    )) {
      throw new Error("APNTTransitionStatementV2 private-transition requires a recovery-packet-carrier output");
    }
    ...
    if (
      ...
      throw new Error(
        "APNTTransitionStatementV2 private-transition must not contain more than one transition-boundary output",
      );
    }
    return;
  }
  ...
  if (statement.consumedLogicalNotes.length !== 1) {
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit requires exactly one consumed logical note");
  }
  if (statement.consumedBackingCells.length === 0) {
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit requires consumed backing cells");
  }
  if (statement.createdLogicalNotes.length !== 0 || statement.createdBackingCells.length !== 0) {
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit must not create private notes or backing cells");
  }
  if (statement.createdBackingCreationScope32 !== null) {
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit must not contain a creation scope");
  }
  if (statement.recoveryPacketBinRoot32 !== null) {
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit must not contain recovery references");
  }
  if (
    ...
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit must not contain aggregator-fee terms");
  }
  if (statement.transactionProjection.inputs.some(
    ...
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit must not contain verifier-only inputs");
  }
  if (
    ...
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit requires one positive transparent-exit output");
  }
  if (
    ...
    throw new Error("APNTTransitionStatementV2 complete-bundle-exit verifier must designate a backing input");
  }
}

function normalizeStatementShape(value: unknown): APNTTransitionStatementV2 {
  assertRecord("APNTTransitionStatementV2", value);
  assertKnownKeys("APNTTransitionStatementV2", value, [
    "version",
    "protocolDomain",
    "network",
    "mode",
  ...
  if (value.version !== APNT_TRANSITION_STATEMENT_V2_VERSION) {
    throw new Error("APNTTransitionStatementV2.version must be 2");
  }
  if (value.protocolDomain !== APNT_TRANSITION_STATEMENT_V2_PROTOCOL_DOMAIN) {
    throw new Error(
      `APNTTransitionStatementV2.protocolDomain must be ${APNT_TRANSITION_STATEMENT_V2_PROTOCOL_DOMAIN}`,
    );
  }
  ...
  const designatedVerifierInputIndex = assertU32(
    "APNTTransitionStatementV2.designatedVerifierInputIndex",
    value.designatedVerifierInputIndex,
  );
  if (designatedVerifierInputIndex >= transactionProjection.inputs.length) {
    throw new Error("APNTTransitionStatementV2.designatedVerifierInputIndex is outside projected inputs");
  }

  const statement: APNTTransitionStatementV2 = Object.freeze({
    version: APNT_TRANSITION_STATEMENT_V2_VERSION,
    protocolDomain: APNT_TRANSITION_STATEMENT_V2_PROTOCOL_DOMAIN,
    network: normalizeNetwork("APNTTransitionStatementV2.network", value.network),
    mode: normalizeMode("APNTTransitionStatementV2.mode", value.mode),
    semanticProfileId32: assertNonzeroBytes32(
      "APNTTransitionStatementV2.semanticProfileId32",
      value.semanticProfileId32,
    ),
    proofRelationId32: assertNonzeroBytes32(
      "APNTTransitionStatementV2.proofRelationId32",
      value.proofRelationId32,
    ),
    sp1ProgramId32: assertNonzeroBytes32(
      "APNTTransitionStatementV2.sp1ProgramId32",
      value.sp1ProgramId32,
    ),
    designatedVerifierInputIndex,
    batchNonce32: assertNonzeroBytes32("APNTTransitionStatementV2.batchNonce32", value.batchNonce32),
    createdBackingCreationScope32: assertOptionalNonzeroBytes32(
      "APNTTransitionStatementV2.createdBackingCreationScope32",
  ...
  if (
    ...
    throw new Error("APNTTransitionStatementV2 public totals must equal projected input/output values");
  }
  if (
    statement.networkFeeSats > projectedInputValue ||
    projectedInputValue - statement.networkFeeSats !== projectedOutputValue
  ) {
    throw new Error("APNTTransitionStatementV2 network fee must equal projected inputs minus outputs");
  }
  return statement;
}

function batchNonceInput(statement: APNTTransitionStatementV2) {
  return {
    ...
    mode: statement.mode,
    semanticProfileId32: statement.semanticProfileId32,
...
function creationScopeInput(statement: APNTTransitionStatementV2) {
  return {
    ...
    createdBackingSkeletons: statement.createdBackingCells.map((cell) => {
      ...
      if (output === undefined) {
        throw new Error("APNTTransitionStatementV2 created backing cell references a missing output");
      }
      return {
 ...
 */
export async function normalizeAPNTTransitionStatementV2(
  value: unknown,
): Promise<APNTTransitionStatementV2> {
  const statement = normalizeStatementShape(value);
  const expectedBatchNonce32 = await deriveAPNTTransitionBatchNonceV2(batchNonceInput(statement));
  if (!bytesEqual(statement.batchNonce32, expectedBatchNonce32)) {
    throw new Error("APNTTransitionStatementV2.batchNonce32 must equal the deterministic v2 batch nonce");
  }
  if (statement.mode === APNT_TRANSITION_MODE_V2.PRIVATE_TRANSITION) {
    const expectedScope32 = await deriveAPNTCreationScopeV2(creationScopeInput(statement));
    if (
      statement.createdBackingCreationScope32 === null ||
      !bytesEqual(statement.createdBackingCreationScope32, expectedScope32)
    ) {
      throw new Error(
        "APNTTransitionStatementV2.createdBackingCreationScope32 must equal the deterministic v2 creation scope",
      );
    }
  }
  return statement;
...
function networkByte(value: APNTTransitionNetworkV2): number {
  return value === "chipnet" ? 0 : value === "mainnet" ? 1 : 2;
}

function modeByte(value: APNTTransitionModeV2): number {
  return value === APNT_TRANSITION_MODE_V2.PRIVATE_TRANSITION ? 0 : 1;
}
...
function encodeNormalizedStatement(statement: APNTTransitionStatementV2): Uint8Array {
  ...
  return concatBytes([
    ...
    ...createdBackingCells,
    encodePresence(statement.recoveryPacketBinRoot32),
    writeU64LE(statement.networkFeeSats),
    writeU64LE(statement.aggregatorServiceFeeSats),
    writeU32LE(
      statement.aggregatorFeeOutputIndex ?? APNT_TRANSITION_AGGREGATOR_FEE_OUTPUT_INDEX_ABSENT_V2,
    ),
...
/** Canonical APNTTSV2 bytes. The Promise reflects deterministic nonce/scope recomputation. */
export async function serializeAPNTTransitionStatementV2(value: unknown): Promise<Uint8Array> {
  return encodeNormalizedStatement(await normalizeAPNTTransitionStatementV2(value));
}
...
class StatementReader {
  ...
  private take(name: string, length: number): Uint8Array {
    if (length < 0 || this.offset > this.bytes.length || this.bytes.length - this.offset < length) {
      throw new Error(`APNTTransitionStatementV2 bytes are truncated at ${name}`);
    }
...
function networkFromByte(value: number): APNTTransitionNetworkV2 {
  if (value === 0) return "chipnet";
  if (value === 1) return "mainnet";
  if (value === 2) return "regtest";
  throw new Error("APNTTransitionStatementV2 bytes have unsupported network");
}

function modeFromByte(value: number): APNTTransitionModeV2 {
  if (value === 0) return APNT_TRANSITION_MODE_V2.PRIVATE_TRANSITION;
  if (value === 1) return APNT_TRANSITION_MODE_V2.COMPLETE_BUNDLE_EXIT;
  throw new Error("APNTTransitionStatementV2 bytes have unsupported mode");
}
...
function readPresence(reader: StatementReader, name: string): Bytes32 | null {
  ...
  throw new Error(`APNTTransitionStatementV2 bytes have invalid ${name} presence tag`);
}

function assertParsedCount(name: string, value: number, cap: number): number {
  if (value > cap) throw new Error(`APNTTransitionStatementV2 bytes have invalid ${name}`);
  return value;
}

/**
 * Parses one exact canonical APNTTSV2 encoding. Encoded collection order must
 * already be canonical; truncation, trailing data, and V1 bytes reject.
 */
export async function parseAPNTTransitionStatementV2(value: unknown): Promise<APNTTransitionStatementV2> {
  if (!(value instanceof Uint8Array)) {
    throw new Error("APNTTransitionStatementV2 bytes must be a Uint8Array");
  }
  const sourceBytes = copyBytes(value);
  const reader = new StatementReader(sourceBytes);
  const magic = reader.fixed("magic", MAGIC.length);
  if (!bytesEqual(magic, MAGIC)) throw new Error("APNTTransitionStatementV2 bytes have invalid magic");
  const version = reader.u8("version");
  if (version !== APNT_TRANSITION_STATEMENT_V2_VERSION) {
    throw new Error("APNTTransitionStatementV2 bytes have unsupported version");
  }
  ...
  try {
    ...
    throw new Error("APNTTransitionStatementV2 bytes have invalid protocol domain encoding");
  }
  const network = networkFromByte(reader.u8("network"));
  const mode = modeFromByte(reader.u8("mode"));
  const semanticProfileId32 = reader.bytes32("semanticProfileId32");
  ...
  const consumedLogicalNoteCount = assertParsedCount(
    ...
    APNT_TRANSITION_STATEMENT_V2_MAX_LOGICAL_NOTES_PER_SIDE,
  );
  const consumedLogicalNotes: APNTTransitionConsumedLogicalNoteV2[] = [];
  ...
  const consumedBackingCellCount = assertParsedCount(
    ...
    APNT_TRANSITION_STATEMENT_V2_MAX_BACKING_CELLS_PER_SIDE,
  );
  const consumedBackingCells: APNTTransitionConsumedBackingCellV2[] = [];
  ...
  const createdLogicalNotes: APNTTransitionCreatedLogicalNoteV2[] = [];
  ...
  const createdBackingCells: APNTTransitionCreatedBackingCellV2[] = [];
  ...
  const recoveryPacketBinRoot32 = readPresence(
    reader,
    "recoveryPacketBinRoot32",
  );
  ...
  const aggregatorFeeOutputIndex =
    encodedAggregatorFeeOutputIndex === APNT_TRANSITION_AGGREGATOR_FEE_OUTPUT_INDEX_ABSENT_V2
      ? null
  ...
  const normalized = await normalizeAPNTTransitionStatementV2({
    version,
    protocolDomain,
    network,
    mode,
  ...
  if (!bytesEqual(sourceBytes, encodeNormalizedStatement(normalized))) {
    throw new Error("APNTTransitionStatementV2 bytes are not in canonical collection order");
  }
  return normalized;
}

/** Domain-separated commitment to the exact canonical APNTTSV2 bytes only. */
export async function deriveAPNTTransitionStatementCommitmentV2(
  value: unknown,
): Promise<Bytes32> {
  return sha256DomainSeparated(
    APNT_TRANSITION_STATEMENT_V2_COMMITMENT_DOMAIN,
    await serializeAPNTTransitionStatementV2(value),
  );
}
