...
The 22 promoted capability specifications live in `spec/`, one file per
...
scenarios in `spec/` only need `WHEN`/`THEN`/`AND`. Look at a few real files
before writing a new one — `spec/apnt-aggregation-transition-output.md` uses
`GIVEN` throughout; `spec/apnt-transition-statement.md` mostly doesn't.
...
`CONTRIBUTING.md` asks for. If you're proposing a new requirement, write it in
...
1. **What spec(s) it touches**, named by file (e.g. `spec/apnt-transition-statement.md`).
2. **The delta itself** — which requirements are added, which scenarios change,
   and, critically, whether it moves a **frozen** identity (see `AGENTS.md`,
...
identity, expect discussion before any code — see `CONTRIBUTING.md` for what's
...
process: the specs in `spec/`, the frozen identities in `AGENTS.md`, and the
